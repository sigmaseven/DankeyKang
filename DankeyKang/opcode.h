...
// 6502 opcode enums. We could programmatically determine
// what opcodes do what, but we're going to need to determine
// opcode byte widths via lookups anyway so it's a moot point.
enum opcode
{
	ADC_IMMEDIATE   = 0x69,
	ADC_ZERO_PAGE   = 0x65,
...
class Opcode
{
	// Get the byte width of an opcode
	static uint8_t GetWidth(uint8_t byte);
	// Get the number of cycles for an opcode
	static uint8_t GetCycles();
};